 ...
 <table class="table" id="myTable">
    ...
    <tbody>
        ...
        @foreach($hostel as $value)
          <tr>
            <td>{{ $value->id }}</td>
            <td>{{ $value->hostelname }}</td>
            <td>{{ $value->location }}</td>
            <td>{{ $value->availableseat}}</td>
            <td>{{ $value->rent }} </td>
            <td>
...
window.onclick = function(event) {
  if (event.target == modal) {
    modal.style.display = "none";
        ...
        ||<a href="hostelupdate/{{$value->id}}" class="btn btn-success">Edit</a>

        ||<a data-toggle="modal" href="hosteldetail/{{$value->id}}" class="btn btn-primary" href="" >Details</a>
